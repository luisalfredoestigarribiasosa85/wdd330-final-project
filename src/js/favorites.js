...
function removeFromFavorites(movieId) {
    ...
    const movieCard = document.querySelector(`[data-id="${movieId}"]`).closest('.col');
    if (movieCard) {
        movieCard.remove();
    }
...
async function getMovieDetails(movieId) {
    try {
        const response = await fetch(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${import.meta.env.VITE_API_MOVIES}`);
